...
calc_nwstats_msm <- function(time.unit = 7,
  ...
  #number of people with a main parter among those who are Y/O with X# casual partners
  ...
  coef.diss.m <- dissolution_coefs(dissolution = diss.main,
                                   duration = durs.main.age / time.unit, #durs.main, vector of main durations;
                                   d.rate = exp.mort)
  ...
  # one-off partners across M/C matrix
...
assign_degree <- function(nw, deg.type, nwstats) {
  ...
  if (deg.type == "main") {
    attr.name <- "deg.main"
    dist.Y <- rowSums(nwstats$deg.mp.Y)
    dist.O <- rowSums(nwstats$deg.mp.O)
  }
  if (deg.type == "pers") {
    attr.name <- "deg.pers"
    dist.Y <- colSums(nwstats$deg.mp.Y)
    dist.O <- colSums(nwstats$deg.mp.O)
  }
